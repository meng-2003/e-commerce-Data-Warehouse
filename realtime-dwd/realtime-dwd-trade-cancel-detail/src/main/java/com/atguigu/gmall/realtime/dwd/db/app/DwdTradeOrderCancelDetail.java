...
/**
 * @author Laoxingjie
 * @description 交易域取消订单事务事实表(与detail表关联相当于四张表合并）
 * @create 2026/4/9 19:29
 **/
public class DwdTradeOrderCancelDetail extends BaseSQLApp {
    public static void main(String[] args) {
        new DwdTradeOrderCancelDetail().start(
                10015,
                4,
                Constant.TOPIC_DWD_TRADE_ORDER_CANCEL_DETAIL
        );
    }
    ...
    public void handle(StreamExecutionEnvironment env, StreamTableEnvironment tEnv) {
        ...
        tEnv.executeSql(
                "create table dwd_trade_order_detail(" +
                        ...
                        "ts bigint " +
                        ")" + SQLUtil.getKafkaDDLSource(Constant.TOPIC_DWD_TRADE_ORDER_CANCEL_DETAIL, Constant.TOPIC_DWD_TRADE_ORDER_DETAIL));
        ...
        tEnv.executeSql(
                "create table dwd_trade_order_cancel_detail(" +
                        ...
                        "split_total_amount string," +
                        "ts bigint " +
                        ")" + SQLUtil.getKafkaDDLSink(Constant.TOPIC_DWD_TRADE_ORDER_CANCEL_DETAIL));

        result.executeInsert(Constant.TOPIC_DWD_TRADE_ORDER_CANCEL_DETAIL);
    }
}
